...
{
    if (adress_to_remove->city)
        delete_string(adress_to_remove->city);
    if (adress_to_remove->street)
        delete_string(adress_to_remove->street);
    if (adress_to_remove->index)
        delete_string(adress_to_remove->index);
    adress_to_remove = NULL;
}

void delete_mail(Mail *mail_to_remove)
{
    if (&(mail_to_remove->delivery_adress))
        delete_adress(&(mail_to_remove->delivery_adress));
    if (mail_to_remove->id)
        delete_string(mail_to_remove->id);
    if (mail_to_remove->create_time)
        delete_string(mail_to_remove->create_time);
    if (mail_to_remove->delivery_time)
        delete_string(mail_to_remove->delivery_time);
    mail_to_remove = NULL;
}

void delete_post(Post *post_to_remove)
{
    if (post_to_remove->branch_adress)
        delete_adress(post_to_remove->branch_adress);
    for (int i = 0; i < post_to_remove->mails_size; ++i)
    {
        delete_mail(&(post_to_remove->mails[i]));
    }
    if (post_to_remove->mails)
    {
        free(post_to_remove->mails);
        post_to_remove->mails = NULL;
    }
    post_to_remove = NULL;
}
...
int my_mails_cmp_main(const void *a, const void *b)
{
    ...
    if (!index_cmp_res)
        return my_string_cmp(mail_1->id, mail_2->id);
...
status_code mail_data_input(String *city, String *street, int *house, String *building, int *flat, String *index)
{
    ...
    if (create_string(city, buff1) == allocate_error || create_string(street, buff2) == allocate_error || create_string(building, buff3) == allocate_error || create_string(index, buff4) == allocate_error)
    {
        return allocate_error;
    }
...
status_code adress_data_input(String *city, String *street, int *house, String *building, int *flat, String *index, double *weight, String *id, String *create_time, String *delivery_time)
{
    ...
    while (getchar() != '\n')
        ;
    if (create_string(id, buff5) == allocate_error || create_string(create_time, buff6) == allocate_error || create_string(delivery_time, buff7) == allocate_error)
    {
        return allocate_error;
    }
...
status_code id_input(String *id)
{
    ...
    char buff[BUFSIZ];
    scanf("%s", buff);
    if (create_string(id, buff) == allocate_error)
        return allocate_error;
    return success;
}
