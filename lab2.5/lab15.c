...
status_code from_base_to_int(const char *number, const int number_size, int base, const bool is_upper, long long int *res)
{
    ...
    *res = 0;
    ...
    {
        int digit = ((isdigit(number[i])) ? number[i] - '0' : (is_upper) ? number[i] - 'A' + 10
                                                                         : number[i] - 'a' + 10);
        if (digit >= base)
        {
            return invalid_parameter;
...
int overfprintf(FILE *stream, const char *format, ...)
{
    ...
    for (int i = 0; i < num_of_args; ++i)
    {
        ...
        while ((isalpha(format[++format_index]) || isdigit(format[format_index]) || (format[format_index] == '%' && format[format_index - 1] == '%') ||
                (format[format_index] == '.' && (isalpha(format[format_index + 1]) || isdigit(format[format_index + 1]) || format[format_index + 1] == '*')) || (format[format_index] == '#') || (format[format_index] == '+') || (format[format_index] == '-') || (format[format_index] == '0')) &&
               (format_index < format_size))
        {
        ...
        else if (!strcmp(flag, "%mf"))
        {
            ...
            total += vfprintf(stream, flag, ptr);
        while (format[format_index] != '%' && (format_index < format_size))
        {
            total += fprintf(stream, "%c", format[format_index]);
            ++format_index;
...
int oversprintf(char *buf, char *format, ...)
{
    ...
    for (int i = 0; i < num_of_args; ++i)
    {
        ...
        else if (strcmp(flag, "%mf") == 0)
        {
            ...
            total += vsprintf(buf + total, flag, ptr);
        while (format[format_index] != '%' && (format_index < format_size))
        {
            total += sprintf(buf + total, "%c", format[format_index]);
            ++format_index;
...
int main()
{
    overfprintf(stdout, "%Cv %Ro %Ro  %CV %mi %mi\n", 1, 1, 1, 1, 1, 1);
    overfprintf(stdout, "%n");
    return 0;
}